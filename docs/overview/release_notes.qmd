...
- Modal extraction
  ...
  - Add `OMAMdofProblem` structure to perform Operational Modal Analysis (OMA) using EMA-based algorithms on half-spectrum data
  - Add `solve` function for `OMAMdofProblem` to extract modal parameters using Covariance-based SSI (`CovSSI`) and Data-based SSI (`DataSSI`) methods
  ...
  - Add `compute_residuals` function for `OMAMdofProblem` to compute the lower and upper residuals of the half-spectrum
