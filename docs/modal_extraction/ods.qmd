...
In practice, ODS are often real-normalized for better interpretability. It consists in converting the complex-valued ODS $\boldsymbol{\Psi}_k$ into a real-valued vector $\bar{\boldsymbol{\Psi}}_k$. This can be achieved using the `real_normalization` function (see [Mode shapes real-normalization](index.qmd#mode-shape-extraction-1)).
