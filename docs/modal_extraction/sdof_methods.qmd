...
```{julia}
#| echo: false
@doc poles_extraction(::AbstractVector, ::AbstractVector, ::CircleFit)
```
:::
...
```{julia}
#| echo: false
@doc modeshape_extraction(H::AbstractArray, freq::AbstractVector, poles::Vector{T}, dpi::Vector{Int}; type = :dis) where {T <: Complex}
```
:::
...
fn_pp, ξn_pp = poles2modal(poles_extraction(H, freq, PeakPicking()))
fn_cf, ξn_cf = poles2modal(poles_extraction(H, freq, CircleFit()))
fn_lsf, ξn_lsf = poles2modal(poles_extraction(H, freq, LSFit()))

# Mode shape extraction
dpi = [1, 2]
ϕid = modeshape_extraction(H, freq, modal2poles(fn_pp, ξn_pp), dpi)
