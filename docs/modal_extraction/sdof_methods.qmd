...
```{julia}
#| echo: false
@doc poles_extraction(::EMASdofProblem, ::CircleFit)
```
:::
...
```{julia}
#| echo: false
@doc modeshape_extraction(::EMASdofProblem, ::Vector{T}, ::Vector{Int}; ::Symbol) where {T <: Complex}
```
:::
...
dpi = [1, 2]
ϕid = modeshape_extraction(prob_sdof, modal2poles(fn_pp, ξn_pp), dpi = dpi)
