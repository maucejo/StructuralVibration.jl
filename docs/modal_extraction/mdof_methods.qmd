...
```{julia}
#| echo: false
@doc poles_extraction(::EMAProblem, ::Int, ::LSCE)
```
:::
...
```{julia}
#| echo: false
@doc stabilization_plot(::StabilizationAnalysis)
```
:::
...
```{julia}
#| echo: false
@doc mode_residues(::EMAProblem, ::Vector{T}) where {T <: Complex}
```
:::
...
```{julia}
#| echo: false
@doc modeshape_extraction(::Array{T, 3}, ::Vector{T} ::LSCE, ::Vector{Int}) where {T <: Complex}
```
:::
...
ϕ_est = modeshape_extraction(res, p_lsce, LSCE(), dpi = dpi, modetype = :emar)[1]
