...
```{julia}
#| echo: false
@doc wave_parameters(b::Bar, freq)
```
:::
...
# Computation of the modal density
md_bar = modal_density(bar)
md_rod = modal_density(rod)
md_strings = modal_density(strings)
md_beam = modal_density(beam, 100.)
```
...
```{julia}
#| echo: false
@doc wave_parameters(p::Plate, freq)
```
:::
...
# Computation of the modal density
md_plate = modal_density(plate)
md_membrane = modal_density(membrane, fmax)
```
...
@doc rayleigh_damping_matrix(M, K, α::Float64, β::Float64)
```
:::
