...
nsig = lines!(ax, t, y, color = :red)
osig = lines!(ax, t, x, linewidth = 2)
xlims!(ax, 0, 1)
Legend(fig[1, 2], [osig, nsig], ["Original signal", "Noisy signal"])
display(fig);
```
...
```{julia}
#| echo: false
@doc acn(x::VecOrMat{Real}, snr_dB::Real, fs::Real, color::Symbol)
```
:::
...
```{julia}
#| echo: false
@doc acn(x::VecOrMat{Complex}, snr_dB::Real, freq::AbstractVector, color::Symbol)
```
:::
